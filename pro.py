...
1. Unique Records:
   ...
   in this table not from the example.if the first name of that record is not similar to other first name record then display in unique records.


2. Duplicate Records:
   Generate a table showing duplicate records based on the smaller length of the first name compared to unique records from the provided records 
   from the user records not from the example i given in the prompt. If a duplicate record has the same first name as a unique record,
   increase the count of the unique record instead of listing it separately. Always give duplicate records from the user provided records not from the example
   i gave in the prompt variable.
...
3. Similar Records:
   Generate a table combining records with similar first names to those in the unique records table. Records with similar first names are those where the first name matches the unique record's first name partially or entirely. Include the count of occurrences for each similar record.
    it should have only one record which is the combination of othere similar records.There should be only one record in this table.
For example:
...
Noah        Hernandez  Maple Street  Detroit  MI     48226  3
...
if user_prompt != last_user_input:
    ...
    # Send user input to Gemini Pro with a prompt
